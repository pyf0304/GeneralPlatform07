...
namespace GeneralPlatform.Webform
{
    ...
    /// </summary>
    public partial class wfmUsers_School_QUDI : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)	//如果本界面是初次被调用，而不是单击按钮事件所调用
            {
                ...
                {
                    //1、为下拉框设置数据源，绑定列表数据
                    wucUsers_School1.SetDdl_DepartmentId();
                    wucUsers_School1.SetDdl_UserStateId();
                    wucUsers_School1.SetDdl_IdentityID();
            ...
            clsCommonSession.UserId, "0001", clsCommonSession.RoleId, "设置口令");
                    switch (vsPotenceFlag)
                    {
                        case "全局级":
                            break;
                        case "项目级":
                            clsQxUsers_SchoolEN objUsers_SchoolEN = clsQxUsers_SchoolBL.GetObjByUserIdCache(clsCommonSession.UserId);
                            //vsDepartmentId = objUsers_SchoolEN.DepartmentId;
                            tvDepartments.Visible = false;
                    ...
                    wucUsers_School1.SetKeyReadOnly(true);
        ...
        ///private clsQxUsers_SchoolEN objUser
        ///{
        ///get
        ///{
        ///clsQxUsers_SchoolEN pobjUser;
        ///pobjUser = (clsQxUsers_SchoolEN)Session["objUser"];
        ///if (pobjUser==null) 
        ///{
        ///pobjUser = null;
        ///}
        ...
        ///set
        ///{
        ///clsQxUsers_SchoolEN pobjUser = value;
        ...
        private string CombineUsersCondion()
        {
            ...
            {
                strWhereCond += " And StuIdTeacherId like '%" + this.txtUserName_q.Text.Trim() + "%'";
            }
            ...
            {
                strWhereCond += string.Format(" And UserId in (Select UserId from {0} where {1}='{2}')",
                    clsQxPrjUserRelationEN._CurrTabName, conQxPrjUserRelation.QxPrjId, this.ddlPrjId_q.SelectedValue );
            }
        ...
        private void BindDg_QxUsers()
        {
            ...
            //2、根据条件串获取该表满足条件的DataTable；
            System.Data.DataTable objDT = clsvQxUsers_SchoolBL.GetDataTable(strWhereCond);
        ...
        private void ShowData(string strUserId)
        {
            ...
            //2、检查该关键字的记录是否存在，如果不存在就返回不显示；
            if (clsQxUsers_SchoolBL.IsExist(strUserId) == false)		//检查该关键字的记录是否存在
            {
            ...
            //3、用提供的关键字初始化一个类对象；
            clsQxUsers_SchoolEN objUsers_SchoolEN = clsQxUsers_SchoolBL.GetObjByUserIdCache(strUserId);
         
            Session.Add("objUsers_SchoolEN", objUsers_SchoolEN);
            //5、把该对象的所有属性显示在界面上，在这里显示在表控件中
            GetDataFromQxUsersClass(objUsers_SchoolEN);
        }
        ...
        private void GetDataFromQxUsersClass(clsQxUsers_SchoolEN pobjUsers)
        {
            wucUsers_School1.UserId = pobjUsers.UserId;
            wucUsers_School1.StuIdTeacherId = pobjUsers.StuTeacherId;
       
            wucUsers_School1.IdentityId = pobjUsers.IdentityId;
            wucUsers_School1.CardNo = pobjUsers.CardNo;

            wucUsers_School1.IsLeaved = pobjUsers.IsLeaved;
            wucUsers_School1.Memo = pobjUsers.Memo;
            if (vsPotenceFlag == "项目级")
            {
                wucUsers_School1.SetAllCtrlFalseExceptIsLeaved();
            }
        }
        /// <summary>
        ...
        protected void btnOKUpd_Click(object sender, System.EventArgs e)
        {
            ...
            clsQxUsers_SchoolEN objUsers_SchoolEN;
            Button btnTemp;
            ...
            switch (strCommandText)
            {
                ...
                case "确认修改":
                    ...
                    //就把逻辑层合并到控制层,
                    objUsers_SchoolEN = (clsQxUsers_SchoolEN)Session["objUsers_SchoolEN"];
                    if (objUsers_SchoolEN == null)
                    {
                        ErrorInformationBL.AddInformation("wfmUsers_QUDI2", "btnOKUpd_Click", "objUsers_SchoolEN为null", "pyf");
                        string strUserId = hidUserId.Value;
                        objUsers_SchoolEN =  clsQxUsers_SchoolBL.GetObjByUserIdCache(strUserId);
                    }
                    UpdateUsersRecordSave(objUsers_SchoolEN);
                    break;
            }
        }
        ///生成从界面层到逻辑层的数据传输函数代码
        ...
        private void PutDataToUsersClass(clsQxUsers_SchoolEN pobjUsers)
        {
            pobjUsers.UserId = wucUsers_School1.UserId;
            pobjUsers.StuTeacherId = wucUsers_School1.StuIdTeacherId;
           
            pobjUsers.IdentityId = wucUsers_School1.IdentityId;
            pobjUsers.CardNo = wucUsers_School1.CardNo;

           
                pobjUsers.IsLeaved = wucUsers_School1.IsLeaved;
           
            pobjUsers.Memo = wucUsers_School1.Memo;
        }
        ...
        protected void btnExportExcel4Dg_Click(object sender, System.EventArgs e)
        {
            ...
            arrColName.Add(convQxUsers_School.UserId); arrCnName.Add("用户ID");
            arrColName.Add(convQxUsers_School.StuTeacherId); arrCnName.Add("用户名");
            arrColName.Add(convQxUsers_School.DepartmentId); arrCnName.Add("部门Id");
            arrColName.Add(convQxUsers_School.DepartmentName); arrCnName.Add("部门名");
            arrColName.Add(convQxUsers_School.UserStateName); arrCnName.Add("用户状态名");
            arrColName.Add(convQxUsers_School.Memo); arrCnName.Add("备注");
        ...
        {
            wucUsers_School1.SetKeyReadOnly(false);
            btnOKUpd.Text = "确认添加";
            //wucUsers_School1.UserId = clsCommForWebForm.G1etMaxStrId("Users","UserId", 18, "");
            if (DepartmentId != "" && DepartmentId != "root")
            {
                wucUsers_School1.DepartmentId = DepartmentId;
            }
            wucUsers_School1.UserStateId = "01";
            DispEditUsersRegion();
        }
        ///插入记录存盘过程代码for C#
        private void AddUsersRecordSave()
        {
            ...
            clsQxUsers_SchoolEN objUsers_SchoolEN;
            //1、检查控件中输入数据类型是否正确
            if (!wucUsers_School1.IsValid(ref strResult))
            {
                lblMsg.Text = "控件中输入数据类型不正确请检查!";
                return;
            }
            //2、定义对象并初始化对象
            //clsQxUsers_SchoolEN objUsers_SchoolEN;	//定义对象
            objUsers_SchoolEN = new clsQxUsers_SchoolEN(wucUsers_School1.UserId);	//初始化新对象
            //3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
            if (clsQxUsers_SchoolBL.IsExist( objUsers_SchoolEN.UserId) == true)	//判断是否有相同的关键字
            {
                strMsg = "关键字字段已有相同的值";
            ...
            //4、把值从界面层传到逻辑层或数据实体层
            PutDataToUsersClass(objUsers_SchoolEN);		//把界面的值传到
            //5、检查传进去的对象属性是否合法
            try
            {
                clsQxUsers_SchoolBL.CheckPropertyNew(objUsers_SchoolEN);
                //6、把数据实体层的数据存贮到数据库中
                clsQxUsers_SchoolBL.AddNewRecordBySql2(objUsers_SchoolEN);
            ...
            lblMsg.Text = "添加记录成功!";
            //7、把新添的记录内容显示在DATAGRID中
            wucUsers_School1.Clear();		//清空控件中内容
            ///恢复<确认添加>变成<添加>
            btnOKUpd.Text = "添加";
            BindDg_QxUsers();
            DispUsersListRegion();
        ...
        private void AddUsersRecordSaveV5()
        {
            ...
            objUsers_SchoolEN = new clsQxUsers_SchoolEN(wucUsers_School1.UserId);	//初始化新对象
            //3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
            if (clsQxUsers_SchoolBL.IsExist(objUsers_SchoolEN.UserId))	//判断是否有相同的关键字
            {
                strMsg = "关键字字段已有相同的值";
                Response.Write("<script>alert('" + strMsg + "')</script>");
                return;
        ...
        ///修改存盘准备过程代码 for C#
        private void UpdateUsersRecordSave(clsQxUsers_SchoolEN objUsers_SchoolEN)
        {
            //操作步骤：
            //1、检查控件中输入数据类型是否正确
            //2、把值从界面层传到逻辑层或数据实体层
            ...
            try
            {
                clsQxUsers_SchoolBL.CheckPropertyNew(objUsers_SchoolEN);
                //6、把数据实体层的数据存贮到数据库中
                 clsQxUsers_SchoolBL.UpdateBySql2(objUsers_SchoolEN);
            }
            catch (Exception objException)
            {
            ...
            Response.Write("<script>alert('" + strMsg + "')</script>");
            lblMsg.Text = "修改记录成功!";
            wucUsers_School1.Clear();//添空控件中的内容
            ///恢复<确认修改>变成<添加>
            btnOKUpd.Text = "添加";
            //5、把修改后的内容显示在DATAGRID中
            BindDg_QxUsers();
        ...
        ///修改存盘准备过程代码 for C#
        private void UpdateUsersRecordSaveV5(clsQxUsers_SchoolEN objUsers_SchoolEN)
        {
            //操作步骤：
            //1、检查控件中输入数据类型是否正确
            //2、把值从界面层传到逻辑层或数据实体层
        ...
        {
            clsQxUsers_SchoolBL.DelRecord(strUserId);
            BindDg_QxUsers();
        }
        ...
        protected void btnSynDataFromPlatform_Click(object sender, EventArgs e)
        {
            ...
            //clsQxUsers_SchoolEN objUsers_SchoolEN = new clsQxUsers_SchoolEN();

            //foreach (TzGeneralPlatformDll2.clsQxUsers_SchoolEN objUsersFromPlat in arrUserObjList)
            //{
            //    objUsers_SchoolEN.UserId = objUsersFromPlat.UserId;


            //    objUsers_SchoolEN.StuIdTeacherId = objUsersFromPlat.StuIdTeacherId;



            //    objUsers_SchoolEN.UserStateId = objUsersFromPlat.UserStateId;
            //    objUsers_SchoolEN.DepartmentId = objUsersFromPlat.DepartmentId;
            //    objUsers_SchoolEN.Memo = "从平台复制";
            //    objUsers_SchoolEN.IsLeaved = objUsersFromPlat.IsLeaved;
            //    if (clsQxUsers_SchoolBL.IsExist(objUsers_SchoolEN.UserId) == false)
            //    {
            //        clsQxUsers_SchoolBL.AddNewRecordBySql2(objUsers_SchoolEN);
