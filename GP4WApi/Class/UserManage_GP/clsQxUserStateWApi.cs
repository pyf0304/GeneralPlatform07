...
/// 静态类
/// </summary>
public static class  clsQxUserStateWApi_Static
{
...
try
{
objQxUserStateEN.sfUpdFldSetStr = objQxUserStateEN.getsfUpdFldSetStr();
clsQxUserStateWApi.CheckPropertyNew(objQxUserStateEN); 
bool bolResult = clsQxUserStateWApi.UpdateRecord(objQxUserStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsQxUserStateWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsQxUserStateEN objQxUserStateEN)
{
//检测记录是否存在
...
try
{
 if (string.IsNullOrEmpty(objQxUserStateEN.UserStateId) == true || clsQxUserStateWApi.IsExist(objQxUserStateEN.UserStateId) == true)
 {
     objQxUserStateEN.UserStateId = clsQxUserStateWApi.GetMaxStrId();
 }
clsQxUserStateWApi.CheckPropertyNew(objQxUserStateEN); 
bool bolResult = clsQxUserStateWApi.AddNewRecord(objQxUserStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsQxUserStateWApi.ReFreshCache();
return bolResult;
}
...
{
clsQxUserStateWApi.CheckPropertyNew(objQxUserStateEN); 
string strUserStateId = clsQxUserStateWApi.AddNewRecordWithMaxId(objQxUserStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsQxUserStateWApi.ReFreshCache();
...
{
clsQxUserStateWApi.CheckPropertyNew(objQxUserStateEN); 
bool bolResult = clsQxUserStateWApi.UpdateWithCondition(objQxUserStateEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsQxUserStateWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class clsQxUserStateWApi
{
private static readonly string mstrApiControllerName = "QxUserStateApi";
...
List<clsQxUserStateEN> arrObjLst = clsQxUserStateWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
{
string strAction = "GetObjByUserStateId";
clsQxUserStateEN objQxUserStateEN;
...
objQxUserStateEN = JsonConvert.DeserializeObject<clsQxUserStateEN>(strJson);
return objQxUserStateEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsQxUserStateEN GetFirstObj(string strWhereCond)
{
...
from objQxUserStateEN in arrQxUserStateObjLstCache
where objQxUserStateEN.UserStateId == strUserStateId 
select objQxUserStateEN;
if (arrQxUserStateObjLst_Sel.Count() == 0)
{
   clsQxUserStateEN obj = clsQxUserStateWApi.GetObjByUserStateId(strUserStateId);
   if (obj != null)
 {
...
//初始化列表缓存
List<clsQxUserStateEN> arrQxUserStateObjLstCache = GetObjLstCache();
IEnumerable <clsQxUserStateEN> arrQxUserStateObjLst_Sel1 =
from objQxUserStateEN in arrQxUserStateObjLstCache
where objQxUserStateEN.UserStateId == strUserStateId 
select objQxUserStateEN;
List <clsQxUserStateEN> arrQxUserStateObjLst_Sel = new List<clsQxUserStateEN>();
foreach (clsQxUserStateEN obj in arrQxUserStateObjLst_Sel1)
{
arrQxUserStateObjLst_Sel.Add(obj);
}
if (arrQxUserStateObjLst_Sel.Count > 0)
{
return arrQxUserStateObjLst_Sel[0].UserStateName;
}
string strErrMsgForGetObjById = string.Format("在QxUserState对象缓存列表中,找不到记录[UserStateId = {0}](函数:{1})", strUserStateId, clsStackTrace.GetCurrFunction());
...
string strErrMsgForGetObjById = string.Format("在QxUserState对象缓存列表中,找不到记录的相关名称[UserStateId = {0}](函数:{1})", strUserStateId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsQxUserStateBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
{
 List<clsQxUserStateEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsQxUserStateEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsQxUserStateEN> GetObjLstByUserStateIdLst(List<string> arrUserStateId)
{
 List<clsQxUserStateEN> arrObjLst; 
string strAction = "GetObjLstByUserStateIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrUserStateId);
...
public static IEnumerable<clsQxUserStateEN> GetObjLstByUserStateIdLstCache(List<string> arrUserStateId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsQxUserStateEN._CurrTabName);
List<clsQxUserStateEN> arrQxUserStateObjLstCache = GetObjLstCache();
IEnumerable <clsQxUserStateEN> arrQxUserStateObjLst_Sel =
from objQxUserStateEN in arrQxUserStateObjLstCache
where arrUserStateId.Contains(objQxUserStateEN.UserStateId)
select objQxUserStateEN;
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsQxUserStateEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsQxUserStateEN> arrObjLst; 
...
try
{
 clsQxUserStateEN objQxUserStateEN = clsQxUserStateWApi.GetObjByUserStateId(strUserStateId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strUserStateId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelQxUserStates(List<string> arrUserStateId)
{
string strAction = "DelQxUserStates";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrUserStateId);
...
{
string strAction = "DelQxUserStatesByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsQxUserStateEN objQxUserStateEN)
{
...
public static string AddNewRecordWithMaxId(clsQxUserStateEN objQxUserStateEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsQxUserStateWApi.ReFreshCache();
var strUserStateId = (string)jobjReturn0["returnStr"];
return strUserStateId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsQxUserStateEN objQxUserStateEN)
{
if (string.IsNullOrEmpty(objQxUserStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objQxUserStateEN.UserStateId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsQxUserStateEN objQxUserStateEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objQxUserStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsQxUserStateEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsQxUserStateEN.AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsQxUserStateEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsQxUserStateEN.AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsQxUserStateEN objInFor in arrObj)
{
//Rows
...
if (clsQxUserStateWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsQxUserStateEN._CurrTabName);
CacheHelper.Remove(strKey);
clsQxUserStateWApi.objCommFun4WApi.ReFreshCache();
}
}
...
var strKey = clsQxUserStateEN._CurrTabName;
List<clsQxUserStateEN> arrQxUserStateObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsQxUserStateEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conQxUserState.UserStateId, Type.GetType("System.String"));
objDT.Columns.Add(conQxUserState.UserStateName, Type.GetType("System.String"));
objDT.Columns.Add(conQxUserState.Memo, Type.GetType("System.String"));
foreach (clsQxUserStateEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conQxUserState.UserStateId] = objInFor[conQxUserState.UserStateId];
objDR[conQxUserState.UserStateName] = objInFor[conQxUserState.UserStateName];
objDR[conQxUserState.Memo] = objInFor[conQxUserState.Memo];
