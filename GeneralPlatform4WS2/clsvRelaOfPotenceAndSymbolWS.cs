...
namespace GeneralPlatform4WS
{
    ...
    /// </summary>
    public class clsvRelaOfPotenceAndSymbolWS
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static vRelaOfPotenceAndSymbolServiceSoapClient GetvRelaOfPotenceAndSymbolServiceSoapClient()
        {
            if (objvRelaOfPotenceAndSymbolServiceSoapClient == null)
            {
                ...
                {
                    strServiceUrl = String.Format("http://{0}/{1}/vRelaOfPotenceAndSymbolService.asmx", clsSysPara.strCurrIPAddressAndPort, clsSysPara.strCurrPrx);
                }
                else
                {
        ...
        /// 构造函数
        /// </summary>
        public clsvRelaOfPotenceAndSymbolWS()
        {
        }
        ...
        {
            vRelaOfPotenceAndSymbolServiceSoapClient objvRelaOfPotenceAndSymbolService = GetvRelaOfPotenceAndSymbolServiceSoapClient();
            DataSet objDS = null;
            try
            {
        ...
        public static DataTable GetDataTable_vRelaOfPotenceAndSymbolVWs(string strWhereCond)
        {
            vRelaOfPotenceAndSymbolServiceSoapClient objvRelaOfPotenceAndSymbolService = GetvRelaOfPotenceAndSymbolServiceSoapClient();
            DataSet objDS = null;
            try
            {
                objDS = objvRelaOfPotenceAndSymbolService.vRelaOfPotenceAndSymbol_GetDataSetV(strWhereCond);
                DataTable objDT = objDS.Tables[0];
                return objDT;
            }
            catch (Exception objException)
            {
                throw new Exception("获取表:vRelaOfPotenceAndSymbol View 数据出错！\r\n" + objException.Message);
            }
        }

        public static DataTable GetvRelaOfPotenceAndSymbolVWs(string strWhereCond)
        {
            vRelaOfPotenceAndSymbolServiceSoapClient objvRelaOfPotenceAndSymbolService = GetvRelaOfPotenceAndSymbolServiceSoapClient();
            DataSet objDS = null;
            try
        ...
        public static bool GetvRelaOfPotenceAndSymbolWs(ref clsvRelaOfPotenceAndSymbolEN objvRelaOfPotenceAndSymbol)
        {
            vRelaOfPotenceAndSymbolServiceSoapClient objvRelaOfPotenceAndSymbolService = GetvRelaOfPotenceAndSymbolServiceSoapClient();
            bool bolResult = objvRelaOfPotenceAndSymbolService.vRelaOfPotenceAndSymbol_GetvRelaOfPotenceAndSymbol(ref objvRelaOfPotenceAndSymbol);
            return bolResult;
        }

        public static List<clsvRelaOfPotenceAndSymbolEN> GetvRelaOfPotenceAndSymbolObjList(string strCondition)
        {
            List<clsvRelaOfPotenceAndSymbolEN> arrObjList = new List<clsvRelaOfPotenceAndSymbolEN>();
            ...
            {
                clsvRelaOfPotenceAndSymbolEN objvRelaOfPotenceAndSymbol = new clsvRelaOfPotenceAndSymbolEN();
                objvRelaOfPotenceAndSymbol.RelaMid = Int32.Parse(objRow["RelaMid"].ToString().Trim()); //流水号
                objvRelaOfPotenceAndSymbol.PrjId = objRow["PrjId"].ToString().Trim(); //项目编号
        ...
        public static bool IsExistRecordWs(string strCondition)
        {
            //检测记录是否存在
            vRelaOfPotenceAndSymbolServiceSoapClient objvRelaOfPotenceAndSymbolService = GetvRelaOfPotenceAndSymbolServiceSoapClient();
            bool bolIsExist = objvRelaOfPotenceAndSymbolService.vRelaOfPotenceAndSymbol_IsExistRecordByCond(strCondition);
            return bolIsExist;
        }
