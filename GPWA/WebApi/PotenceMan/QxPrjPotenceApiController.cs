...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrPotenceIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByPotenceIdLstCache")]
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsQxPrjPotenceEN objQxPrjPotenceCond = new();
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 通过JSON对象来添加记录对象
 /// 调用方法: POST /api/QxPrjPotenceApi/AddNewRecordWithMaxId
 /// 在Body区传输objQxPrjPotenceEN的JSON对象
...
public ActionResult AddNewRecordWithMaxId([FromBody]clsQxPrjPotenceEN objQxPrjPotenceEN)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxPrjPotenceJSONObj = clsJSON.GetJsonFromObj(objQxPrjPotenceEN);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 ...
 {
    clsQxPrjPotenceBL.AccessFldValueNull(objQxPrjPotenceEN);
string strPotenceId = objQxPrjPotenceEN.AddNewRecordWithMaxId();
return Ok(new { errorId = 0, errorMsg = "", returnStr =  strPotenceId });
 }
 catch (Exception objException)
 {
