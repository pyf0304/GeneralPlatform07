...
try
{
objQxProjectsEN.sfUpdFldSetStr = objQxProjectsEN.getsfUpdFldSetStr();
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr(this clsQxProjectsEN objQxProjectsEN)
{
//检测记录是否存在
...
try
{
 if (string.IsNullOrEmpty(objQxProjectsEN.QxPrjId) == true || clsQxProjectsWApi.IsExist(objQxProjectsEN.QxPrjId) == true)
 {
     objQxProjectsEN.QxPrjId = clsQxProjectsWApi.GetMaxStrId();
 }
clsQxProjectsWApi.CheckPropertyNew(objQxProjectsEN); 
...
clsQxProjectsEN objQxProjectsEN;
...
objQxProjectsEN = JsonConvert.DeserializeObject<clsQxProjectsEN>(strJson);
return objQxProjectsEN;
}
else
{
...
from objQxProjectsEN in arrQxProjectsObjLstCache
...
{
 List<clsQxProjectsEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsQxProjectsEN>>(strJson);
return arrObjLst;
}
else
{
...
public static IEnumerable<clsQxProjectsEN> GetObjLstByQxPrjIdLstCache(List<string> arrQxPrjId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsQxProjectsEN._CurrTabName);
List<clsQxProjectsEN> arrQxProjectsObjLstCache = GetObjLstCache();
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsQxProjectsEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsQxProjectsEN> arrObjLst; 
...
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strQxPrjId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
var strQxPrjId = (string)jobjReturn0["returnStr"];
return strQxPrjId;
}
else
{
...
{
if (string.IsNullOrEmpty(objQxProjectsEN.sfUpdFldSetStr) == true)
{
...
if (clsQxProjectsWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsQxProjectsEN._CurrTabName);
CacheHelper.Remove(strKey);
clsQxProjectsWApi.objCommFun4WApi.ReFreshCache();
}
}
...
//初始化列表缓存
var strWhereCond = "";
var strKey = clsQxProjectsEN._CurrTabName;
List<clsQxProjectsEN> arrQxProjectsObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
