...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrQxCodeTypeIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByQxCodeTypeIdLstCache")]
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrQxCodeTypeObjLst });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxCodeTypeApi/GetObjLstByPager_Cache?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 arrQxCodeTypeObjLst = clsQxCodeTypeBL.GetObjLstByPagerCache(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrQxCodeTypeObjLst });
 ...
 /// <summary>
 /// 根据条件获取范围内的对象列表
 /// 调用方法: GET /api/QxCodeTypeApi/GetObjLstByRange?strOrderBy=v1&intMaxNum=v2&intMinNum=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 arrQxCodeTypeObjLst = clsQxCodeTypeBL.GetObjLstByRange(objRangePara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxCodeTypeEN objQxCodeTypeCond = new();
