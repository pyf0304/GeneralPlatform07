    ...
    public class clsvUsersWS
    {
        ...
        public static vUsersServiceSoapClient GetvUsersServiceSoapClient()
        {
            if (objvUsersServiceSoapClient == null)
            {
                ...
                {
                    strServiceUrl = String.Format("http://{0}/{1}/vUsersService.asmx", clsSysPara.CurrIPAddressAndPort, clsSysPara.CurrPrx);
                }
                else
                {
        ...
        public static clsvUsersEN GetFirstvUsersObj_S(string strCondition)
        {
            System.Data.DataTable objDT = null;
            objDT = GetDataTable_vUsersWs(strCondition);
            if (objDT.Rows.Count == 0)
            ...
            clsvUsersEN objvUsers = new clsvUsersEN();
            objvUsers.UserId = objDT.Rows[0]["UserId"].ToString().Trim(); //用户ID
            objvUsers.UserName = objDT.Rows[0]["UserName"].ToString().Trim(); //用户名
            objvUsers.DepartmentId = objDT.Rows[0]["DepartmentId"].ToString().Trim(); //部门Id
            objvUsers.DepartmentName = objDT.Rows[0]["DepartmentName"].ToString().Trim(); //部门名
            objvUsers.UserStateId = objDT.Rows[0]["UserStateId"].ToString().Trim(); //用户状态Id
            objvUsers.UserStateName = objDT.Rows[0]["UserStateName"].ToString().Trim(); //用户状态名
            objvUsers.Memo = objDT.Rows[0]["Memo"].ToString().Trim(); //备注
            objvUsers.DepartmentAbbrName = objDT.Rows[0]["DepartmentAbbrName"].ToString().Trim(); //名称缩写
            objvUsers.DepartmentTypeId = objDT.Rows[0]["DepartmentTypeId"].ToString().Trim(); //部门类型ID
            objvUsers.DepartmentTypeName = objDT.Rows[0]["DepartmentTypeName"].ToString().Trim(); //部门类型名
            objvUsers.IdentityDesc = objDT.Rows[0]["IdentityDesc"].ToString().Trim(); //身份描述
            objvUsers.IdentityID = objDT.Rows[0]["IdentityID"].ToString().Trim(); //身份编号
            objvUsers.SubjectTypeName = objDT.Rows[0]["SubjectTypeName"].ToString().Trim(); //学科类型
            objvUsers.UpDepartmentId = objDT.Rows[0]["UpDepartmentId"].ToString().Trim(); //所属部门号
            objvUsers.UpDepartmentName = objDT.Rows[0]["UpDepartmentName"].ToString().Trim(); //上级部门
            objvUsers.BeginYearMonth = objDT.Rows[0]["BeginYearMonth"].ToString().Trim(); //开始年月
            objvUsers.EndYearMonth = objDT.Rows[0]["EndYearMonth"].ToString().Trim(); //结束年月
            objvUsers.GradeBaseID = objDT.Rows[0]["GradeBaseID"].ToString().Trim(); //年级代号
            objvUsers.GradeBaseName = objDT.Rows[0]["GradeBaseName"].ToString().Trim(); //年级名称
            objvUsers.id_GradeBase = objDT.Rows[0]["id_GradeBase"].ToString().Trim(); //年级流水号
            objvUsers.IsOffed = com.taishsoft.commdb.clsGeneralTab.TransNullToBool_S(objDT.Rows[0]["IsOffed"].ToString().Trim()); //是否毕业
            objDT = null;
            return objvUsers;
        }

        public static clsvUsersEN GetvUsersObjByUserId_S(string strUserId)
        {
        ...
        public static bool IsExistRecordWs(string strCondition)
        {
            //检测记录是否存在
            vUsersServiceSoapClient objvUsersService = GetvUsersServiceSoapClient();
            bool bolIsExist = objvUsersService.vUsers_IsExistRecord(strCondition);
            return bolIsExist;
        }
