...
clsStackTrace.GetCurrClassFunction()); 
 ...
 if (clsDataBaseTypeWApi.IsExist(objDataBaseTypeEN.DataBaseTypeId) == true)
 {
...
clsDataBaseTypeEN objDataBaseTypeEN;
...
objDataBaseTypeEN = JsonConvert.DeserializeObject<clsDataBaseTypeEN>(strJson);
return objDataBaseTypeEN;
}
else
{
...
from objDataBaseTypeEN in arrDataBaseTypeObjLstCache
...
{
 List<clsDataBaseTypeEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsDataBaseTypeEN>>(strJson);
return arrObjLst;
}
else
{
...
public static IEnumerable<clsDataBaseTypeEN> GetObjLstByDataBaseTypeIdLstCache(List<string> arrDataBaseTypeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsDataBaseTypeEN._CurrTabName);
List<clsDataBaseTypeEN> arrDataBaseTypeObjLstCache = GetObjLstCache();
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsDataBaseTypeEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsDataBaseTypeEN> arrObjLst; 
...
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strDataBaseTypeId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
var strDataBaseTypeId = (string)jobjReturn0["returnStr"];
return strDataBaseTypeId;
}
else
{
...
//初始化列表缓存
var strWhereCond = "";
var strKey = clsDataBaseTypeEN._CurrTabName;
List<clsDataBaseTypeEN> arrDataBaseTypeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
