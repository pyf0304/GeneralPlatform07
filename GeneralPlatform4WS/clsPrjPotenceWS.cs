...
namespace GeneralPlatform4WS
{
    ...
    public class clsQxPrjPotenceWS
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static PrjPotenceServiceSoapClient GetQxPrjPotenceServiceSoapClient()
        {
            if (objPrjPotenceServiceSoapClient == null)
            {
                ...
                {
                    strServiceUrl = String.Format("http://{0}/{1}/PrjPotenceService.asmx", clsSysPara.CurrIPAddressAndPort, clsSysPara.CurrPrx);
                }
                else
                {
        ...
        {
            //为数据源为表的下拉框设置内容
            System.Data.DataTable objDT = GetDataTable_QxPrjPotenceWs("1=1");
            clsQxPrjPotenceEN objPrjPotence;
            //初始化一个对象列表
            List<clsQxPrjPotenceEN> PrjPotenceList = new List<clsQxPrjPotenceEN>();
        ...
        ///			 并显示在ListView中。
        /// </summary>
        public static int BindLv_QxPrjPotence(System.Windows.Forms.ListView lvPrjPotence, string strWhereCond)
        {
            ///	操作步骤：(共4步)
            ///	1、组合界面条件串；
            ...
            List<clsQxPrjPotenceEN> arrPrjPotenceObjList;
            ///	2、根据条件串获取该表满足条件的DataTable；
            arrPrjPotenceObjList =  clsQxPrjPotenceWS.GetQxPrjPotenceObjList(strWhereCond);
            ///	3、设置ListView的列头信息
            ...
            ///	4、设置ListView的Item信息。即把所有记录显示在ListView中
            foreach (clsQxPrjPotenceEN objPrjPotence in arrPrjPotenceObjList)
            {
        ...
        public static DataTable GetDataTable_QxPrjPotenceWs(string strWhereCond)
        {
            PrjPotenceServiceSoapClient objPrjPotenceService = GetQxPrjPotenceServiceSoapClient();
            DataSet objDS = null;
            try
            {
        ...
        public static DataTable GetDataTable_QxPrjPotenceVWs(string strWhereCond)
        {
            PrjPotenceServiceSoapClient objPrjPotenceService = GetQxPrjPotenceServiceSoapClient();
            DataSet objDS = null;
            try
        ...
        public static bool GetQxPrjPotenceWs(ref clsQxPrjPotenceEN objPrjPotence)
        {
            PrjPotenceServiceSoapClient objPrjPotenceService = GetQxPrjPotenceServiceSoapClient();
            bool bolResult = objPrjPotenceService.PrjPotence_GetQxPrjPotence(ref objPrjPotence);
            return bolResult;
        }

        public static List<clsQxPrjPotenceEN> GetQxPrjPotenceObjList(string strCondition)
        {
            List<clsQxPrjPotenceEN> arrObjList = new List<clsQxPrjPotenceEN>();
            System.Data.DataTable objDT = null;
            objDT = GetDataTable_QxPrjPotenceWs(strCondition);
            ...
            foreach (DataRow objRow in objDT.Rows)
            {
                clsQxPrjPotenceEN objPrjPotence = new clsQxPrjPotenceEN();
                objPrjPotence.PotenceId = objRow["PotenceId"].ToString().Trim(); //权限ID
                objPrjPotence.QxPrjId = objRow[clsQxProjectsEN.con_QxPrjId].ToString().Trim(); //项目编号
        ...
        public static bool AddNewRecordBySql2Ws(clsQxPrjPotenceEN objPrjPotence)
        {
            PrjPotenceServiceSoapClient objPrjPotenceService = GetQxPrjPotenceServiceSoapClient();
            try
            {
        ...
        public static bool UpdateBySql2Ws(clsQxPrjPotenceEN objPrjPotence)
        {
            PrjPotenceServiceSoapClient objPrjPotenceService = GetQxPrjPotenceServiceSoapClient();
            try
            {
        ...
        public static bool IsExistRecordWs(string strCondition)
        {
            //检测记录是否存在
            PrjPotenceServiceSoapClient objPrjPotenceService = GetQxPrjPotenceServiceSoapClient();
            bool bolIsExist = objPrjPotenceService.PrjPotence_IsExistRecord(strCondition);
            return bolIsExist;
        }
