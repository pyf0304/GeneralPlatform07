...
/// 静态类
/// </summary>
public static class  clsPageDispModeWApi_Static
{
...
try
{
objPageDispModeEN.sfUpdFldSetStr = objPageDispModeEN.getsfUpdFldSetStr();
clsPageDispModeWApi.CheckPropertyNew(objPageDispModeEN); 
bool bolResult = clsPageDispModeWApi.UpdateRecord(objPageDispModeEN);
...
clsStackTrace.GetCurrClassFunction()); 
 ...
 if (clsPageDispModeWApi.IsExist(objPageDispModeEN.PageDispModeId) == true)
 {
...
{
clsPageDispModeWApi.CheckPropertyNew(objPageDispModeEN); 
bool bolResult = clsPageDispModeWApi.AddNewRecord(objPageDispModeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPageDispModeWApi.ReFreshCache();
return bolResult;
}
catch (Exception objException)
{
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objPageDispModeEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsPageDispModeEN objPageDispModeEN)
{
try
{
clsPageDispModeWApi.CheckPropertyNew(objPageDispModeEN); 
string strPageDispModeId = clsPageDispModeWApi.AddNewRecordWithMaxId(objPageDispModeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPageDispModeWApi.ReFreshCache();
...
{
clsPageDispModeWApi.CheckPropertyNew(objPageDispModeEN); 
bool bolResult = clsPageDispModeWApi.UpdateWithCondition(objPageDispModeEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPageDispModeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class enumPageDispModeWA
{
 /// <summary>
 /// 弹出框
 /// </summary>
...
public const string FullPage_05 = "05";
 ...
 /// </summary>
public class clsPageDispModeWApi
{
private static readonly string mstrApiControllerName = "PageDispModeApi";
...
List<clsPageDispModeEN> arrObjLst = GetObjLst("1=1");
...
{
if (!Object.Equals(null, objPageDispModeEN.PageDispModeId) && GetStrLen(objPageDispModeEN.PageDispModeId) > 2)
{
 throw new Exception("字段[页面显示模式Id]的长度不能超过2!");
}
if (!Object.Equals(null, objPageDispModeEN.PageDispModeName) && GetStrLen(objPageDispModeEN.PageDispModeName) > 50)
{
 throw new Exception("字段[页面显示模式名称]的长度不能超过50!");
}
if (!Object.Equals(null, objPageDispModeEN.PageDispModeENName) && GetStrLen(objPageDispModeEN.PageDispModeENName) > 50)
{
 throw new Exception("字段[页面显示模式英文名]的长度不能超过50!");
}
if (!Object.Equals(null, objPageDispModeEN.Memo) && GetStrLen(objPageDispModeEN.Memo) > 1000)
{
...
{
string strAction = "GetObjByPageDispModeId";
clsPageDispModeEN objPageDispModeEN;
...
objPageDispModeEN = JsonConvert.DeserializeObject<clsPageDispModeEN>(strJson);
return objPageDispModeEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsPageDispModeEN GetFirstObj(string strWhereCond)
{
...
from objPageDispModeEN in arrPageDispModeObjLstCache
where objPageDispModeEN.PageDispModeId == strPageDispModeId 
select objPageDispModeEN;
if (arrPageDispModeObjLst_Sel.Count() == 0)
{
   clsPageDispModeEN obj = clsPageDispModeWApi.GetObjByPageDispModeId(strPageDispModeId);
   if (obj != null)
 {
...
string strKey = string.Format("{0}", clsPageDispModeEN._CurrTabName);
List<clsPageDispModeEN> arrPageDispModeObjLstCache = GetObjLstCache();
IEnumerable <clsPageDispModeEN> arrPageDispModeObjLst_Sel1 =
from objPageDispModeEN in arrPageDispModeObjLstCache
where objPageDispModeEN.PageDispModeId == strPageDispModeId 
select objPageDispModeEN;
List <clsPageDispModeEN> arrPageDispModeObjLst_Sel = new List<clsPageDispModeEN>();
foreach (clsPageDispModeEN obj in arrPageDispModeObjLst_Sel1)
{
arrPageDispModeObjLst_Sel.Add(obj);
}
if (arrPageDispModeObjLst_Sel.Count > 0)
{
return arrPageDispModeObjLst_Sel[0].PageDispModeName;
}
string strErrMsgForGetObjById = string.Format("在PageDispMode对象缓存列表中,找不到记录[PageDispModeId = {0}](函数:{1})", strPageDispModeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsPageDispModeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在PageDispMode对象缓存列表中,找不到记录的相关名称[PageDispModeId = {0}](函数:{1})", strPageDispModeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsPageDispModeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
{
 List<clsPageDispModeEN> arrObjLst; 
...
arrObjLst = JsonConvert.DeserializeObject<List<clsPageDispModeEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsPageDispModeEN> GetObjLstByPageDispModeIdLst(List<string> arrPageDispModeId)
{
 List<clsPageDispModeEN> arrObjLst; 
string strAction = "GetObjLstByPageDispModeIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrPageDispModeId);
...
public static IEnumerable<clsPageDispModeEN> GetObjLstByPageDispModeIdLstCache(List<string> arrPageDispModeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsPageDispModeEN._CurrTabName);
List<clsPageDispModeEN> arrPageDispModeObjLstCache = GetObjLstCache();
IEnumerable <clsPageDispModeEN> arrPageDispModeObjLst_Sel =
from objPageDispModeEN in arrPageDispModeObjLstCache
where arrPageDispModeId.Contains(objPageDispModeEN.PageDispModeId)
select objPageDispModeEN;
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsPageDispModeEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsPageDispModeEN> arrObjLst; 
...
try
{
 clsPageDispModeEN objPageDispModeEN = clsPageDispModeWApi.GetObjByPageDispModeId(strPageDispModeId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strPageDispModeId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelPageDispModes(List<string> arrPageDispModeId)
{
string strAction = "DelPageDispModes";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrPageDispModeId);
...
{
string strAction = "DelPageDispModesByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsPageDispModeEN objPageDispModeEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsPageDispModeEN>(objPageDispModeEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithMaxId(clsPageDispModeEN objPageDispModeEN)
{
...
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
var strPageDispModeId = (string)jobjReturn0["returnStr"];
return strPageDispModeId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsPageDispModeEN objPageDispModeEN)
{
if (string.IsNullOrEmpty(objPageDispModeEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objPageDispModeEN.PageDispModeId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsPageDispModeEN objPageDispModeEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objPageDispModeEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsPageDispModeEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsPageDispModeEN.AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsPageDispModeEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsPageDispModeEN.AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsPageDispModeEN objInFor in arrObj)
{
//Rows
...
if (clsPageDispModeWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsPageDispModeEN._CurrTabName);
CacheHelper.Remove(strKey);
clsPageDispModeWApi.objCommFun4WApi.ReFreshCache();
}
}
...
//初始化列表缓存
var strWhereCond = "";
var strKey = clsPageDispModeEN._CurrTabName;
List<clsPageDispModeEN> arrPageDispModeObjLstCache = CacheHelper.GetCache(strKey, () => { return GetObjLst(strWhereCond); });
...
public static DataTable GetDataTableByObjLst(List<clsPageDispModeEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conPageDispMode.PageDispModeId, Type.GetType("System.String"));
objDT.Columns.Add(conPageDispMode.PageDispModeName, Type.GetType("System.String"));
objDT.Columns.Add(conPageDispMode.PageDispModeENName, Type.GetType("System.String"));
objDT.Columns.Add(conPageDispMode.Memo, Type.GetType("System.String"));
foreach (clsPageDispModeEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conPageDispMode.PageDispModeId] = objInFor[conPageDispMode.PageDispModeId];
objDR[conPageDispMode.PageDispModeName] = objInFor[conPageDispMode.PageDispModeName];
objDR[conPageDispMode.PageDispModeENName] = objInFor[conPageDispMode.PageDispModeENName];
objDR[conPageDispMode.Memo] = objInFor[conPageDispMode.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4PageDispMode : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsPageDispModeWApi.ReFreshThisCache();
}
}
