...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <param name = "arrMenuSetIdLst">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
[HttpPost("GetObjLstByMenuSetIdLstCache")]
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrQxPrjMenuSetObjLst });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/QxPrjMenuSetApi/GetObjLstByPager_Cache?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 arrQxPrjMenuSetObjLst = clsQxPrjMenuSetBL.GetObjLstByPagerCache(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrQxPrjMenuSetObjLst });
 ...
 /// <summary>
 /// 根据条件获取范围内的对象列表
 /// 调用方法: GET /api/QxPrjMenuSetApi/GetObjLstByRange?strOrderBy=v1&intMaxNum=v2&intMinNum=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 arrQxPrjMenuSetObjLst = clsQxPrjMenuSetBL.GetObjLstByRange(objRangePara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsQxPrjMenuSetEN objQxPrjMenuSetCond = new();
