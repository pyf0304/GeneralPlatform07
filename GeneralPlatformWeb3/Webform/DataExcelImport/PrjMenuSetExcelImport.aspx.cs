...
{
    public partial class PrjMenuSetExcelImport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //clsQxProjectsBL.BindDdl_id_Projects(ddlid_Major_q);
                //BindDdl_id_Grade(ddlid_Grade_q);
                strSortPrjMenuSetBy = "MenuSetId Asc";
                //2、显示无条件的表内容在DATAGRID中
                BindGv_QxPrjMenuSet();
                this.gvPrjMenuSet.Visible = false;
            }
        ...
        //绑定gridview 数据
        public void GetDataPrjMenuSet()
        {
            System.Data.DataTable dtable = BindGv_Excel();
            //删除空行空列
            removeEmpty(dtable);
            this.gvPrjMenuSet_Disp.DataSource = dtable;
            this.gvPrjMenuSet_Disp.DataBind();
        }
        ...
        protected void btnImport_Click(object sender, EventArgs e)
        {
            ...
            if (dtable.Rows.Count > 0)
            {
                //删除空行
                removeEmpty(dtable);
                bool i = clsPubFun.AddDateExcelToPrjMenuSet(dtable, ref lblmess, this);
                if (i == true)
                {
                    clsCommonJsFunc.Alert(this, "操作成功！");
                    this.stuGV.Visible = false;
                    this.gvPrjMenuSet.Visible = true;
                }
                else
                {
                    this.gvPrjMenuSet.Visible = false;
        ...
        ///删除记录过程代码for C#
        protected void DeletePrjMenuSetRecord(string strid_QxPrjMenuSet)
        {
            try
            {
                clsQxPrjMenuSetBL.DelRecord(strid_QxPrjMenuSet);
            }
            catch (Exception objException)
            {
        ...
        {
            List<string> lstKeyValues = clsCommForWebForm.GetAllCheckedItemFromGv(gvPrjMenuSet, "chkCheckRec", "id_QxPrjMenuSet");
            ...
            try
            {
                foreach (string strKeyValue in lstKeyValues)
                {
                    ...
                    string strid_QxPrjMenuSet = strKeys[0];
                    clsQxPrjMenuSetBL.DelRecord(strid_QxPrjMenuSet);
                }
            }
            catch (Exception objException)
        ...
        {
            string strid_QxPrjMenuSet = gvPrjMenuSet.DataKeys[e.RowIndex]["id_QxPrjMenuSet"].ToString();
            DeletePrjMenuSetRecord(strid_QxPrjMenuSet);
        }
        ...
        protected void gvPrjMenuSet_Disp_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            //System.Data.DataTable dtable = BindGv_Excel();
            this.gvPrjMenuSet.PageIndex = e.NewPageIndex;
            GetDataPrjMenuSet();
