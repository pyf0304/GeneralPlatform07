...
/// 静态类
/// </summary>
public static class  clsTabStateWApi_Static
{
...
try
{
objTabStateEN.sfUpdFldSetStr = objTabStateEN.getsfUpdFldSetStr();
clsTabStateWApi.CheckPropertyNew(objTabStateEN); 
bool bolResult = clsTabStateWApi.UpdateRecord(objTabStateEN);
...
clsStackTrace.GetCurrClassFunction()); 
 ...
 if (clsTabStateWApi.IsExist(objTabStateEN.TabStateId) == true)
 {
...
{
clsTabStateWApi.CheckPropertyNew(objTabStateEN); 
bool bolResult = clsTabStateWApi.AddNewRecord(objTabStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTabStateWApi.ReFreshCache();
return bolResult;
}
catch (Exception objException)
{
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objTabStateEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsTabStateEN objTabStateEN)
{
try
{
clsTabStateWApi.CheckPropertyNew(objTabStateEN); 
string strTabStateId = clsTabStateWApi.AddNewRecordWithMaxId(objTabStateEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTabStateWApi.ReFreshCache();
...
{
clsTabStateWApi.CheckPropertyNew(objTabStateEN); 
bool bolResult = clsTabStateWApi.UpdateWithCondition(objTabStateEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTabStateWApi.ReFreshCache();
return bolResult;
}
...
public const string ToBeConsidered_04 = "04";
 ...
 /// </summary>
public class clsTabStateWApi
{
private static readonly string mstrApiControllerName = "TabStateApi";
...
List<clsTabStateEN> arrObjLst = GetObjLst("1=1");
...
List<clsTabStateEN> arrObjLst = clsTabStateWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
{
if (!Object.Equals(null, objTabStateEN.TabStateId) && GetStrLen(objTabStateEN.TabStateId) > 2)
{
 throw new Exception("字段[表状态ID]的长度不能超过2!");
}
if (!Object.Equals(null, objTabStateEN.TabStateName) && GetStrLen(objTabStateEN.TabStateName) > 30)
{
 throw new Exception("字段[表状态名称]的长度不能超过30!");
}
if (!Object.Equals(null, objTabStateEN.TabStateENName) && GetStrLen(objTabStateEN.TabStateENName) > 30)
{
 throw new Exception("字段[表状态英文名]的长度不能超过30!");
}
if (!Object.Equals(null, objTabStateEN.Memo) && GetStrLen(objTabStateEN.Memo) > 1000)
{
...
{
string strAction = "GetObjByTabStateId";
...
objTabStateEN = JsonConvert.DeserializeObject<clsTabStateEN>(strJson);
return objTabStateEN;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsTabStateEN GetFirstObj(string strWhereCond)
{
...
//该表没有使用Cache,不需要生成[GetTabStateNameByTabStateIdCache]函数;(in AutoGCLib.WA_Access4CSharp:Gen_4WA_GetRecNameByKeyCache)
...
arrObjLst = JsonConvert.DeserializeObject<List<clsTabStateEN>>(strJson);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsTabStateEN> GetObjLstByTabStateIdLst(List<string> arrTabStateId)
{
 List<clsTabStateEN> arrObjLst; 
string strAction = "GetObjLstByTabStateIdLst";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrTabStateId);
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsTabStateEN> GetObjLstByPagerCache(stuPagerPara objPagerPara)
{
 List<clsTabStateEN> arrObjLst; 
...
try
{
 clsTabStateEN objTabStateEN = clsTabStateWApi.GetObjByTabStateId(strTabStateId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strTabStateId.ToString(), out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelTabStates(List<string> arrTabStateId)
{
string strAction = "DelTabStates";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrTabStateId);
...
{
string strAction = "DelTabStatesByCond";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["strWhereCond"] = strWhereCond
};
...
public static bool AddNewRecord(clsTabStateEN objTabStateEN)
{
string strAction = "AddNewRecord";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJson = clsJSON.GetJsonFromObj<clsTabStateEN>(objTabStateEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out string strResult, out string strErrMsg) == true)
{
JObject jobjReturn0 = JObject.Parse(strResult);
if ((int)jobjReturn0["errorId"] == 0)
...
public static string AddNewRecordWithMaxId(clsTabStateEN objTabStateEN)
{
...
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
var strTabStateId = (string)jobjReturn0["returnStr"];
return strTabStateId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsTabStateEN objTabStateEN)
{
if (string.IsNullOrEmpty(objTabStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
objTabStateEN.TabStateId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsTabStateEN objTabStateEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objTabStateEN.sfUpdFldSetStr) == true)
{
string strMsg = string.Format("修改时,修改标志串为空,请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsTabStateEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsTabStateEN.AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsTabStateEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsTabStateEN.AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsTabStateEN objInFor in arrObj)
{
//Rows
...
if (clsTabStateWApi.objCommFun4WApi != null) 
{
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
string strKey = string.Format("{0}", clsTabStateEN._CurrTabName);
CacheHelper.Remove(strKey);
clsTabStateWApi.objCommFun4WApi.ReFreshCache();
}
}
...
public static DataTable GetDataTableByObjLst(List<clsTabStateEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conTabState.TabStateId, Type.GetType("System.String"));
objDT.Columns.Add(conTabState.TabStateName, Type.GetType("System.String"));
objDT.Columns.Add(conTabState.TabStateENName, Type.GetType("System.String"));
objDT.Columns.Add(conTabState.Memo, Type.GetType("System.String"));
foreach (clsTabStateEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conTabState.TabStateId] = objInFor[conTabState.TabStateId];
objDR[conTabState.TabStateName] = objInFor[conTabState.TabStateName];
objDR[conTabState.TabStateENName] = objInFor[conTabState.TabStateENName];
objDR[conTabState.Memo] = objInFor[conTabState.Memo];
...
return objDT;
 ...
 /// </summary>
public class  clsCommFun4WA4TabState : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsTabStateWApi.ReFreshThisCache();
}
}
